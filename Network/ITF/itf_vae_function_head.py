...
class VAEFunctionHead(nn.Module):
    def __init__(self, 
                 inp_size:int, 
                 num_functions: int,
                 attention:str = "hard",
        ...
        self.hf= nn.Sequential(nn.Linear(inp_size//2, num_functions))
        # self.hf2 = nn.Sequential(nn.Linear(inp_size//2, num_functions))
    ...
    def forward(self, inp):
        x = self.head_functions(inp)
        head_func_mu =self.hf(x)
        # head_func_logvar = self.hf2(x)
...
if __name__ == '__main__':
    ...
    # get model
    net = VAEFunctionHead(50, 10, "hard", 3).to(DEVICE)
